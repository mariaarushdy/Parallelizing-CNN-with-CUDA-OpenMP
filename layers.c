...
void conv_forward(conv_layer_t *l, volume_t **inputs, volume_t **outputs, int start, int end) {
    ...
    for (int i = start; i <= end; i++) {
        ...
        #pragma omp parallel for collapse(3)
        for (int f = 0; f < output_depth; f++) {
            for (int tile_y = 0; tile_y < output_height; tile_y += TILE_SIZE_Y) {
                for (int tile_x = 0; tile_x < output_width; tile_x += TILE_SIZE_X) {
                    for (int dy = 0; dy < TILE_SIZE_Y; dy++) {
                        for (int dx = 0; dx < TILE_SIZE_X; dx++) {
                            int out_y = tile_y + dy;
                            int out_x = tile_x + dx;
                            int y = -pad + out_y * stride;
                            int x = -pad + out_x * stride;
                ...
                for (int f = 0; f < output_depth; f++) {
                    ...
                    for (int fy = 0; fy < l->filter_width; fy++) {
                        ...
                        for (int fx = 0; fx < l->filter_width; fx++) {
                            ...
                            if (in_y >= 0 && in_y < in->height && in_x >= 0 && in_x < in->width) {
                                int weight_offset = ((l->filter_width * fy) + fx) * in->depth;
                                int input_offset = ((in->width * in_y) + in_x) * in->depth;

                                for (int fd = 0; fd < in->depth; fd++) {
                                    sum += l->filters[f]->weights[weight_offset + fd] *
                                           in->weights[input_offset + fd];
                                }
                            }
                        sum += l->filters[f]->weights[weight_offset + fd] *
                                           in->weights[input_offset + fd];
        ...
        for (int x = 0; x < l->input_width; x++) {
            for (int y = 0; y < l->input_height; y++) {
                for (int d = 0; d < l->input_depth; d++) {
                    ...
                    double value = (inputs[i]->weights[((inputs[i]->width * y) + x) * inputs[i]->depth + d] < 0.0) ? 0.0 : inputs[i]->weights[((inputs[i]->width * y) + x) * inputs[i]->depth + d];
        ...
        for(int d = 0; d < l->output_depth; d++) {
            ...
            for(int out_x = 0; out_x < l->output_width; x += l->stride, out_x++) {
                ...
                for(int out_y = 0; out_y < l->output_height; y += l->stride, out_y++) {
                    ...
                    out->weights[((out->width * out_y) + out_x) * out->depth + d] = max;
                }
            }
        }
    }
